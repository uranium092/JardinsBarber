...
router.use(async(req,res,next)=>{
   ...
   let response=await fetch("http://3.144.244.103/barbero/obtener/"+req.cookies.id);
...
router.get("/datosBarbero", async (req,res)=>{
   let request=await fetch("http://3.144.244.103/barbero/obtener/"+req.cookies.id);
...
router.get("/cambiar", (req,res)=>{
   ...
   fetch("http://3.144.244.103/barbero/obtener/"+id).then((e)=>{
      e.json().then((cuerpo)=>{
...
router.get("/actualizarDatosBarbero", async(req, res)=>{
   let request=await fetch("http://3.144.244.103/barbero/obtener/"+req.cookies.id);
    ...
    if(response.profilePicture==""){
        response.profilePicture="http://3.144.244.103/images/same/profile.png"
    }else{
        response.profilePicture="http://3.144.244.103/images/"+response.idBarber+"/"+response.profilePicture;
    }
    let data={
...
router.get("/verAgenda", async(req,res)=>{
   ...
   res.render("Barbero/renderizacionAgenda",{data:responseBarbero, currentBarber:req.cookies.id});
});
