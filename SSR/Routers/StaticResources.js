...
router.get('/index', async (req, res) => {
  if (req.cookies.id == undefined) {
    ...
    if (req.cookies.rol == 'usuario') {
      let request = await fetch('http://localhost:8080/barbero/obtenerTodos');
      let requestUsuario = await fetch(
        'http://localhost:8080/cita/obtenerCitasPorUsuario/' + req.cookies.id
      );
