...
router.post('/registrar', (req, res) => {
  fetch('http://localhost:8080/usuario/insertarUsuario', {
    method: 'POST',
...
router.get('/datosUsuario', (req, res) => {
  fetch('http://localhost:8080/usuario/obtenerCliente/' + req.cookies.id).then((e) => {
    e.json().then((cuerpo) => {
...
router.get('/traerPassword', (req, res) => {
  ...
  fetch('http://localhost:8080/usuario/obtenerCliente/' + id).then((e) => {
    e.json().then((cuerpo) => {
      console.log(cuerpo);
...
router.get('/infoBarbero/:id', (req, res) => {
  fetch('http://localhost:8080/barbero/obtener/' + req.params.id)
    .then((response) => {
...
router.get('/verAgenda/:id', async (req, res) => {
  let r = await fetch('http://localhost:8080/cita/obtenerCitasPorUsuario/' + req.cookies.id);
  let b = await r.json();
  if (b.length > 0) return res.redirect('/index');
  let key = req.params.id;
  let responseBarbero = await fetch('http://localhost:8080/barbero/obtener/' + key);
  if (responseBarbero.status == 404) return res.redirect('/index');
  let bodyBarbero = await responseBarbero.json();

  let responseCita = await fetch('http://localhost:8080/cita/obtenerCitasPorBarbero/' + key);
  ...
  res.render('Usuario/verAgendaBarbero', {
    state: state,
    citas: bodyCita,
    barbero: bodyBarbero,
    idUserSession: req.cookies.id,
  });
});
...
router.get('/agendarCita/:idDate/:idUser', (req, res) => {
  fetch('http://localhost:8080/cita/agendar/' + req.params.idDate + '/' + req.params.idUser, {
    method: 'POST',
  }).then((r) => {
    r.json().then((b) => {
