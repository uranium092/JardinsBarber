...
router.get("/barberos", async (req, res)=>{
    let response=await fetch("http://3.144.244.103/barbero/obtenerTodos");
...
router.get("/actualizarpassword", (req,res)=>{
    fetch("http://3.144.244.103/barbero/obtener/"+req.cookies.id).then((e)=>{
        e.json().then((cuerpo)=>{
            console.log(cuerpo);
...
router.get("/infoBarbero/:id", async(req,res)=>{
    ...
    try{
       let response=await fetch("http://3.144.244.103/barbero/obtener/"+key);
