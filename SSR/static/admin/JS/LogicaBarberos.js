$(document).ready(function () {
  ...
  $('.delete').click(function (e) {
    let action = confirm(
      'Se eliminará a ' + e.target.dataset.name.toUpperCase() + '. ¿Estás Seguro?'
    );
    if (action) {
      fetch('http://localhost:8080/cita/datosCitaPorBarbero/' + e.target.dataset.key).then(
        (res) => {
