...
window.addEventListener("load",()=>{
    ...
    document.querySelector("form").addEventListener("submit",(e)=>{
        ...
        if(photo.files.length==0){ //Sin foto-archivo seleccionado
            fetch("http://3.144.244.103/barbero/actualizarBarberoSinFoto", {headers:{'Content-Type':"application/json"}, method:"PUT", body:JSON.stringify(dcc)}).finally(()=>{
                document.location.href="/admin/datosAdmin";
            });
        }else{
            ...
            fetch("http://3.144.244.103/barbero/actualizarBarberoConFoto", {method:"PUT", body:multipart}).finally(()=>{
                document.location.href="/admin/datosAdmin";
            });
        }
