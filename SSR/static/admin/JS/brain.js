$(document).ready(function(){
    $("form").submit(function(e){
        e.preventDefault();
        let body={nameBarber:$("#name").val(), phoneBarber:$("#phone").val(), emailBarber:$("#email").val(),passwordBarber:$("#pass").val(), descriptionBarber:$("#description").val(), profilePicture:"", role:"barbero"};
        if(document.querySelector("#photo").files.length==0){
            body.profilePicture="";
            fetch("http://3.144.244.103/barbero/insertarBarberoSinFoto", {method:"POST", headers:{'Content-Type':"application/json"}, body:JSON.stringify(body)}).finally(async()=>{
                try{ 
            ...
            fetch("http://3.144.244.103/barbero/insertarBarberoConFoto", {method:"POST", body:multipart}).finally(async()=>{
                document.location.href="/admin/barberos";
            });;
        }
    });
