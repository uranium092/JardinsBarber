window.addEventListener("load",()=>{
    ...
    document.querySelector("form").addEventListener("submit",(e)=>{
        e.preventDefault();
        if(passwordBarber.value==passwordBarberConfirm.value && passwordBarber.value.length>0 && passwordBarberConfirm.value.length>0){
            let dcc = {
            ...
            fetch("http://3.144.244.103/barbero/actualizarBarberoSinFoto",{method:"PUT", headers:{'Content-Type':"application/json"}, body:JSON.stringify(dcc)}).then( (response)=>{
